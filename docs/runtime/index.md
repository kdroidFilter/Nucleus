...
Libraries that use JNI (`decorated-window`, `darkmode-detector`, `system-color`, `native-ssl`) require `-keep` rules for their native bridge classes — these are handled by the plugin automatically.
