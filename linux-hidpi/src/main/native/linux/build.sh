#!/bin/bash
# Compiles nucleus_hidpi_linux.c into per-architecture shared libraries (x64 + aarch64).
# The outputs are placed in the JAR resources so they ship with the library.
#
# Prerequisites: gcc, libX11-dev, libdl (usually in glibc), JDK with JNI headers.
# Usage: ./build.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SRC="$SCRIPT_DIR/nucleus_hidpi_linux.c"
...
if [ "$HOST_ARCH" = "x86_64" ]; then
    mkdir -p "$OUT_DIR_X64"
    gcc "${COMMON_FLAGS[@]}" \
        -o "$OUT_DIR_X64/libnucleus_linux_hidpi_jni.so" "$SRC"
    echo "Built x64:"
    ls -lh "$OUT_DIR_X64/libnucleus_linux_hidpi_jni.so"
elif [ "$HOST_ARCH" = "aarch64" ]; then
    mkdir -p "$OUT_DIR_AARCH64"
    gcc "${COMMON_FLAGS[@]}" \
        -o "$OUT_DIR_AARCH64/libnucleus_linux_hidpi_jni.so" "$SRC"
    echo "Built aarch64:"
    ls -lh "$OUT_DIR_AARCH64/libnucleus_linux_hidpi_jni.so"
else
    echo "WARNING: Unsupported host architecture: $HOST_ARCH" >&2
    exit 1
fi

# Attempt cross-compilation (optional, non-fatal)
if [ "$HOST_ARCH" = "x86_64" ]; then
    if command -v aarch64-linux-gnu-gcc &>/dev/null; then
        ...
        if [ -f "$OUT_DIR_AARCH64/libnucleus_linux_hidpi_jni.so" ]; then
    ...
    if command -v x86_64-linux-gnu-gcc &>/dev/null; then
        ...
        if [ -f "$OUT_DIR_X64/libnucleus_linux_hidpi_jni.so" ]; then
