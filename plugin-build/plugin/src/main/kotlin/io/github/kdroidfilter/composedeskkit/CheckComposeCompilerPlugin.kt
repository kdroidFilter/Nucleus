...
import io.github.kdroidfilter.composedeskkit.internal.KOTLIN_ANDROID_PLUGIN_ID
import io.github.kdroidfilter.composedeskkit.internal.KOTLIN_JS_PLUGIN_ID
...
import io.github.kdroidfilter.composedeskkit.internal.Version
import io.github.kdroidfilter.composedeskkit.internal.ideaIsInSyncProvider
...
internal const val minimalSupportedKgpVersionError =
    "e: Configuration problem: " +
...
private fun Project.checkComposeCompilerPlugin(kgp: KotlinBasePlugin) {
    ...
    if (Version.fromString(kgpVersion) < Version.fromString(minimalSupportedKgpVersion)) {
        if (ideaIsInSync) {
            logger.error(minimalSupportedKgpVersionError)
        } else {
            error(minimalSupportedKgpVersionError)
        }
    } else {
